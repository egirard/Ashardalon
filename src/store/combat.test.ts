...
import { rollD20, resolveAttack, arePositionsAdjacent, getAdjacentMonsters, getMonsterAC, canLevelUp, levelUpHero, calculateDamage, checkHealingSurgeNeeded, useHealingSurge, checkPartyDefeat, canUseActionSurge, useActionSurge } from './combat';
...
describe('canUseActionSurge', () => {
  it('should return true when HP < maxHp and surges available', () => {
    const heroState: HeroHpState = {
      heroId: 'quinn',
      currentHp: 5,
      maxHp: 8,
    ...
    expect(canUseActionSurge(heroState, resources)).toBe(true);
  });

  it('should return false when HP = maxHp (no need to heal)', () => {
    const heroState: HeroHpState = {
      heroId: 'quinn',
      currentHp: 8,
      maxHp: 8,
    ...
    expect(canUseActionSurge(heroState, resources)).toBe(false);
  });
  ...
  it('should return true even when HP = 0 (forced surge needed)', () => {
    const heroState: HeroHpState = {
      heroId: 'quinn',
      currentHp: 0,
      maxHp: 8,
...
describe('useActionSurge', () => {
  it('should add surge value to current HP', () => {
    const heroState: HeroHpState = {
      heroId: 'quinn',
      currentHp: 3,
      maxHp: 8,
    ...
    const result = useActionSurge(heroState, resources);

    expect(result.heroState.currentHp).toBe(7); // 3 + 4 = 7
    expect(result.resources.healingSurges).toBe(1);
  ...
  it('should cap HP at maxHp', () => {
    ...
    expect(result.heroState.currentHp).toBe(8); // 6 + 4 = 10, capped at 8
    expect(result.hpRestored).toBe(2); // Only restored 2 HP
  });
  ...
  it('should work with hero at 0 HP (forced usage)', () => {
    const heroState: HeroHpState = {
      heroId: 'quinn',
      currentHp: 0,
      maxHp: 8,
    ...
    expect(result.heroState.currentHp).toBe(4); // 0 + 4 = 4
    expect(result.resources.healingSurges).toBe(0);
    expect(result.hpRestored).toBe(4);
  });
});
